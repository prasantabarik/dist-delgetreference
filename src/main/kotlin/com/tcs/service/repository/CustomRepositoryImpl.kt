...
//        return mongoTemplate.find(query, DeliveryMomentModel::class.java)
        val base = DataBaseConnectionConfig().mongoTemplate()?.find(query, BaseModel::class.java)
...
//        )
    ...
    val delSched = DataBaseConnectionConfig().mongoTemplate()?.find(toPrint, DeliveryScheduleModel::class.java)
    DataBaseConnectionConfig().mongo()?.close()
...
//            DeliveryChannel::class.java)
    ...
    val delChannel = DataBaseConnectionConfig().mongoTemplate()?.find(toPrint, DeliveryChannel::class.java)
    DataBaseConnectionConfig().mongo()?.close()
...
//            DeliveryChannel::class.java)
    ...
    fun getLogisticChannelRepo(storeNumber: Long?,deliveryStream:Int?,startDate:String):
        ...
        val delLogChannel = DataBaseConnectionConfig().mongoTemplate()?.find(toPrint, LogisticChannel::class.java)
        DataBaseConnectionConfig().mongo()?.close()

        return delLogChannel
//        return mongoTemplate.find(toPrint,
